...
public class FallbackDoubleFunctionTest {
	@Test public void full() {
		...
		assertEquals("value", new FallbackDoubleFunction<String>(full, fallback).apply(1));
	...
	@Test public void empty() {
		...
		@SuppressWarnings("unchecked") Supplier<String> fallback = mock(Supplier.class);
		when(fallback.get()).thenReturn("fallback");
		assertEquals("fallback", new FallbackDoubleFunction<String>(empty, fallback).apply(1));
